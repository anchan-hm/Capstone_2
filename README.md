# **EmporiUm Sales Territory Analysis 2**



### **Project Overview:**

For this project, I have been assigned to analyze sales data for two EmporiUm bookstore territories: **Texas** and **South** **Carolina**. The goal is to answer the managers' questions and to compare how each territory performed, and use data insights to recommend where marketing should focus next quarter.



The analysis was completed in **Jupyter Notebook** using **Python**, **Pandas**, **NumPy, Matplotlib** and **scikit-learn**. I broke down each question into markdown and code blocks, performed calculations and created charts to visualize store and category performance.



### **Key Points From Analysis:**

* Identified the territory managers and listed the store IDs and cities for each territory.
* Calculated monthly in-store total revenue for both South Carolina and Texas.
* Ranked store performance to see which stores were the top performers.
* Matched customer IDs with rewards IDs to find the top customers in each territory.
* Analyzed monthly transactions and revenue by product category to understand which products are most popular and where growth opportunities might be.



### **Tools \& Libraries Used:**

* Python
* Pandas
* NumPy
* Matplotlib
* scikit-learn (sklearn)
* Jupyter Notebook





### **Video Presentation:**

[Click Link Here](https://yearuptemp-my.sharepoint.com/:v:/g/personal/achanthadara_my_yearupunited_org/IQC1DR1F3tcdQYbymQUeLxrFAb7dj6lfinsUf2vsI_vC7kw?nav=eyJyZWZlcnJhbEluZm8iOnsicmVmZXJyYWxBcHAiOiJPbmVEcml2ZUZvckJ1c2luZXNzIiwicmVmZXJyYWxBcHBQbGF0Zm9ybSI6IldlYiIsInJlZmVycmFsTW9kZSI6InZpZXciLCJyZWZlcnJhbFZpZXciOiJNeUZpbGVzTGlua0NvcHkifX0&e=WT3oUv)



# **Appendix:**



### **Bonus Overview:**

For the bonus portion of this project, I have expanded my analysis by exploring additional questions about monthly performance, year-over-year revenue, and long-term trends beyond my assigned territories. I used the same workflow from the main project to break down each question, perform calculations, and create visualizations to support my findings.

